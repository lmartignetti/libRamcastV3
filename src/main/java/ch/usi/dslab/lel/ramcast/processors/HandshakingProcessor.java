...
public class HandshakingProcessor {
  ...
  public void initHandshaking(RamcastEndpoint endpoint) throws IOException {
    if (logger.isDebugEnabled()) logger.debug("Init handshaking for endpoint {}", endpoint);
    ...
    if (logger.isDebugEnabled())
      logger.debug("[HS] Step 1 CLIENT Sending: group={}, node= {}, remoteHeadBlock addr={} lkey={} capacity={}",
              buffer.getInt(4),
              buffer.getInt(8),
              buffer.getLong(12),
              buffer.getInt(20),
  ...
  public void handleHandshakeMessage(RamcastEndpoint endpoint, ByteBuffer buffer)
    ...
    if (ticket == RamcastConfig.MSG_HS_C1) { // msg step 1 sent from client
      if (logger.isDebugEnabled())
        logger.debug("[HS] Step 1 SERVER Receiving from [{}/{}] remoteHeadBlock addr={} lkey={} capacity={}",
                buffer.getInt(4),
                buffer.getInt(8),
                buffer.getLong(12),
                buffer.getInt(20),
      ...
      if (logger.isDebugEnabled())
        logger.debug("[HS] Step 1 SERVER setClientMemoryBlockOfRemoteHead={}",
      ...
      RamcastMemoryBlock memorySegmentBlock = getNodeMemorySegmentBlock(
