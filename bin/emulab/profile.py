...
TBURL = "http://longle255.com/libramcast-node-setup.tar.gz"
TBCMD = "sudo chmod +x /tmp/setup-node.sh && /tmp/setup-node.sh 2>&1 | tee /tmp/node-setup.log.$(date +'%Y%m%d%H%M%S')"
...
pc.defineParameter("phystype",  "Optional physical node type",
                   ...
                   longDescription="Specify a physical node type (r320, c6220,etc) " +
...
for i in range(params.nodeCount):
    ...
    if params.osImage and params.osImage != "default":
        ...
        # We want the node type default image if default is selected
    ...
    node.addService(pg.Execute(shell="bash", command=TBCMD))
